...
def main(coverage_file: TextIO, output: str) -> None:
...
def plot_chroms_coverage(ax: Axes, cov_df: DataFrame) -> Tuple[Axes, List[int]]:
    """Plots the coverage for each chromosome and set x ticks"""
    pos_offset = 0
    tick_pos = list()
    chroms = cov_df.chrom.unique()

    ticks_distance = cov_df.shape[0] / 40
    ...
    for chrom in chroms:
        ...
        tick_pos += get_ticks_pos(chrom_len, pos_offset, ticks_distance)

        ax.plot(chrom_df["plot_pos"], chrom_df["cov"], label=chrom, linewidth=0.5)
...
def format_subplot(ax: Axes, legend_cols: int) -> Axes:
...
def format_axes(ax: Axes, tick_pos: List[int]) -> Axes:
...
def get_ticks_pos(length: int, offset: int, distance: float) -> List[int]:
